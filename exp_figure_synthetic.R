...
for (i in seq_along(sim_names)) {
  ...
  p <- ggplot(d_combined_summary[simulator_name == sim_name], 
              aes(x = n, y = diff_te_mean, color = algorithm)) +
    geom_point() +
    geom_line() +
    scale_size_identity() + 
    scale_linewidth_identity() +
    geom_errorbar(aes(ymin = diff_te_mean - diff_te_se, 
                      ymax = diff_te_mean + diff_te_se), width = 0.2) +
    labs(title = sim_name) + 
...
final_plot <- wrap_plots(plots, ncol = 3) &
  ...
  labs(y = "Mean Absolute Error of Treatment Effect")
...
ggsave("./figures/diff_te_mean_combined.png", plot = final_plot, width = 6, height = 5, dpi = 300)
...
ggsave("./figures/jaccard_combined.png", plot = final_plot, width = 6, height = 5, dpi = 300)
